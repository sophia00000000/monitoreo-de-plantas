...
async function cargarEspecies() {
    const respuesta = await fetch(`${API_URL}/plantas`);
...
function pintarCriterios() {
    criterios.innerHTML = configuracion[especie.value].map((criterio) => `
        <label>${criterio.nombre} (${criterio.unidad})
            <input id="${criterio.nombre}" type="number" min="0" step="0.1" required>
        </label>`).join("");
}
...
async function evaluar() {
    ...
    const cuerpo = {
        ...
        ...Object.fromEntries(configuracion[especie.value].map((criterio) => [
            criterio.nombre, document.querySelector(`#${criterio.nombre}`).value,
        ])),
    };

    const respuesta = await fetch(`${API_URL}/evaluar`, {
        method: "POST",
    ...
    const datos = await respuesta.json();
    if (!respuesta.ok) {
        resultado.innerHTML = `<p class="error">${datos.mensaje || datos.error}</p>`;
        return;
    }
    ...
    resultado.innerHTML = `
        ...
        <div class="recomendaciones"><h3>Recomendaciones</h3>${datos.recomendaciones.length
...
cargarEspecies().then(() => {
    document.querySelector("#humedad").value = 40;
    document.querySelector("#iluminacion").value = 300;
    document.querySelector("#temperatura").value = 22;
    return evaluar();
}).catch(() => {
    resultado.className = "resultado";
